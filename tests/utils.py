...
import os
import openai
from embedia import LLM, ChatLLM, Message, Tokenizer, Tool
load_dotenv()
...
#     async def _reply(self, messages: Message) -> Message:
...
class OpenAILLM(LLM):
    ...
    async def _complete(self, prompts: str) -> str:
        openai.api_key = os.getenv("OPENAI_API_KEY")
...
#                          args={"command: str": "The bash command to be run"})
...
#         super().__init__(name="Bash Shell",
#                          desc="Run bash commands",
#                          args={"query: str": "The bash command to be run"})
#         self.executable = executable
#         self.timeout = timeout
...
#         super().__init__(embedding_model=OpenAIEmbedding())
